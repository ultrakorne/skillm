...
### C6 — Unbundle the CLI; separate CLI and app releases
User decision (supersedes the Bundled CLI of C1, the version coupling of C4/C5, and C5's CI job,
already removed): the app no longer carries a skillm binary. It drives the CLI the user installed
(`install.sh`, `/usr/local/bin` or `~/.local/bin`; Homebrew's `/opt/homebrew/bin` too), and the
CLI and the app are versioned and released independently. The user keeps them compatible by
shipping an app update whenever a CLI change breaks it; `api_version` is the guard.
Scope: `macos/` (SkillmKit, Skillm, tests, project.yml, scripts), `docs/`, and Go only where
something assumes a bundled CLI.
- **No Bundled CLI.** Drop `scripts/build-cli.sh`, the build phase and `Contents/Helpers/skillm`,
  and every check of them (tests, `release.sh`'s universal-binary and version checks).
  `SkillmBinary` finds the CLI: `$SKILLM_BIN` (debug builds), then `/usr/local/bin`,
  `~/.local/bin`, `/opt/homebrew/bin`, then the login shell's `command -v skillm` (bounded by a
  timeout). It is looked up again at each launch check, so an install or upgrade made in a
  terminal is picked up.
- **CLI missing** → the menu says "skillm CLI is not installed" with **Install skillm CLI**, which
  runs the bundled `install.sh` (latest release; the version pinning to a Bundled CLI goes away)
  and then re-runs the launch checks. Settings' command-line tool row keeps its Install button and
  shows the path and version when found.
- **`api_version` mismatch** → CLI older than the app supports: "skillm CLI is too old" with
  **Upgrade skillm CLI** (runs `skillm upgrade`, plain, not JSON, since the protocol is not
  usable), then re-check. CLI newer than the app supports: "This app is too old for skillm X"
  with **Check for app update** (Sparkle, with its UI).
- **Two upgrades, one dot.** "Upgrade skillm CLI to X" appears when the Refresh cache's self
  entry says a newer CLI exists (`skillm upgrade --json`, then re-read version and status).
  "Upgrade app and restart" appears when Sparkle found a newer app. Sparkle now checks on its
  own (silently, in the background, on the app's refresh interval, and when Refresh runs),
  never from the CLI's self status. The red dot is `status.badge` (skill updates or a newer CLI)
  or a found app update.
- **Releases.** CLI: unchanged (`vX.Y.Z` tags, goreleaser in CI, the latest release, which
  `install.sh` and `skillm upgrade` read). App: tags `app-vX.Y.Z`, built and published from a
  Mac by `release.sh app-vX.Y.Z` and `publish-release.sh app-vX.Y.Z`; its GitHub release is
  created with `--latest=false`, never needs CLI archives, and its app version is independent.
  The Sparkle feed moves off `releases/latest`: a fixed release tagged `macos-appcast`
  (`--latest=false`) holds `appcast.xml`, which `publish-release.sh` replaces
  (`gh release upload --clobber`); `SUFeedURL` is
  `https://github.com/ultrakorne/skillm/releases/download/macos-appcast/appcast.xml`, and the
  appcast items point at the `app-vX.Y.Z` release zips.
- Go: `internal/selfupdate`'s bundled method and `skillm upgrade`'s bundle refusal may stay (an
  app bundle may still hold a copy someone put there) but no doc may claim the app bundles one.
Acceptance: xcodebuild tests for CLI discovery (each location, none, the login-shell fallback
with a fake shell), the missing-CLI and both mismatch paths, the CLI-upgrade item and the dot;
`release.sh --dry-run app-v0.0.1` passes on this Mac; `docs/` describes the new shape; the Go
gate passes.
...
| C2 | done | fa841175d4984089a0a529b971a03c739d7a4f40 | 57ad9e3 (feature docs in `d15641d`, this log in the next commit) | **Deviations:** (1) the scheduler waits with `Task.sleep` on the suspending clock instead of a `Timer`: time asleep does not count, a wake cancels the hourly wait, the wake tick runs 30 s after the wake and the hourly loop restarts from it. (2) Each scheduled tick runs `config get` then `refresh --if-due` in one operation, so the toggle follows a terminal `config set`; launch is version → status → config get → refresh --if-due. (3) `Notice` carries an origin (`.user`/`.background`); background notices clear on the next successful tick or status read. (4) The update summary follows the CLI's "up to date" rule and names source_missing, status_not_saved and drift_check_skipped. The optional `UNUserNotificationCenter` notification was not built. View skills, Add skill, Settings and Upgrade and restart stay with C3/C4.<br>**Review:** 6 confirmed (2 medium: wake tick fired at wake; sticky background error notice; 4 low), all fixed in 57ad9e3; 67 xcodebuild tests pass.<br>**Notes for C3/C4:** every quit the app starts must go through `AppDelegate.quit()`; never call `NSApp.terminate` from a Task, main-queue block or XPC callback while a command runs (the app hangs). For Sparkle use `updater(_:shouldPostponeRelaunchForUpdate:untilInvokingBlock:)` to `await model.shutdown()` before relaunching; keep `.terminateLater` (`.terminateCancel` would cancel a logout). The C2 hand-off agent returned before its forked project-documentation agent finished; the orchestrator committed this row. |
| C3 | done | 09ff5382d88aedd418a42010925e2fc85a434b5f | 8a5a7cc48ed450e9ac0d91a162e9effc672d0557 (docs + this log in the next commit) | **Deviations:** (1) Add skill is `source inspect <src> [--ref R]` → checklist → Global or `--project <dir>` → `install --ref <branch> --commit <sha> --events` (B3), not `--ref <commit>`; `commit_mismatch` inspects again and keeps the ticked skills. A local folder works too (typed or Choose…; `~` expanded, a relative path refused). (2) `foreign_files` does **not** retry with `--force`, as the plan says: its sheet offers Overwrite (`--yes`, canonical slots only) or Skip These (`--skip-foreign`), per the A4/B2 notes. `--force` is offered only by a separate "Take Over Links" sheet, shown when a result (or a failure after some skills landed) carries `link_refused` warnings, and it re-runs the same request narrowed to those skills. (3) Disabling an agent asks first (a sheet), and the last enabled agent cannot be turned off; the interval picker offers 1 h to 1 week plus any configured value. (4) The command-line tool goes in `/usr/local/bin` only when it is writable, else `~/.local/bin` (no admin prompt); a link into another skillm app or a dangling one is replaced, anything else refused. (5) `AppModel` gained `read` (list, agent ls, source inspect, config get run beside the one changing command; a quit waits for them), `change` (a window command, then a status re-read and `installsVersion += 1`) and `isReady`; each window has a model in SkillmKit (`SkillsModel`, `AddSkillModel`, `SettingsModel`) owned by `AppDelegate`. Uninstall passes the confirmed projects as `--confirmed-root`, asks again on `needs_confirm` and offers "Uninstall Anyway" (`--force`) on `needs_force`.<br>Review round (`8a5a7cc`) fixed all 5 confirmed findings: (1, high) an answer to the foreign-files question rebuilt the install from the form as edited since; the question now stores the request (`InstallRequest`) and the retry uses only it, the form is disabled while an install runs, and a question about a replaced inspection is dropped; (2) the `--force` choice keyed on `link_refused` inside `foreign_files`, which the real CLI never sends (`core.InstallSkills` refuses before any link is made), so it was replaced by the refused-links sheet above; `internal/protocol/testdata/error_foreign_files.json` now carries an `agent_skipped` warning instead, and `fake-skillm` has `FAKE_SKILLM_LINK_REFUSED`; (3) a `config get`/`agent ls` that overlapped a `config set`/`agent set` could put old values back: write counters (bumped at a write's start and end) drop such reads; (4) Disable and both install questions closed without doing anything when another command was running: they stay open, greyed out with "Waiting for the running command…" (Disable is now a sheet, not a confirmation dialog, which always closes); (5) a Skills or Settings window restored at launch stayed on "skillm is not ready yet.": both load again when `isReady` turns true (`restorationBehavior(.disabled)` needs macOS 15). No timing test for (3). 110 xcodebuild tests pass. Docs: new `docs/features/macos-app/windows.md`; the JSON API install paragraph now says `link_refused` comes on the result, never with `foreign_files`.<br>**Notes for C4:** the "Upgrade and restart" item belongs in `MenuContent` next to the window items; a Sparkle relaunch must `await model.shutdown()` first (C2 note) and every quit goes through `AppDelegate.quit()`. The command-line tool is a symlink into `skillm.app/Contents/Helpers/skillm`, so a Sparkle update in place keeps it working; a moved app leaves it dangling, and Settings then shows Install again. Worth a manual check (not verified in review): with an earlier inspection loaded, Return in the Add Skill Source field might trigger the default Install button rather than Read Skills.<br>**User actions:** none for C3. The Developer ID, notarization and Sparkle actions recorded under A3 still apply to C4/C5 (team 6LH2JMGD3J). |
| C4 | done | 262a34d307103bb7dba7a3277e95d7b72ac9d646 | 6bca17c12b71180b3e52516d00273421b0810409 (implementation `bfc8b8c`; docs + this log in the next commit) | **Deviations:** (1) Sparkle 2.10.0 (SPM, `exactVersion`) with `SUEnableAutomaticChecks` off: Sparkle never checks on its own schedule. `AppUpgrade` asks it silently (`checkForUpdateInformation`) once per cache `checked_at` when `self.available` is set, or (review fix) when the **bundled** CLI's own release lookup failed (`self.error`), so a GitHub API failure or rate limit does not hide a valid appcast. The item then waits for Sparkle's answer instead of showing on `self.available` alone, as the B4 note allows: `self.available` is not a trigger of its own, since a GitHub release does not mean its appcast and signed app are uploaded. The Auto refresh setting and interval therefore cover both checks. (2) The appcast is served from the release (`https://github.com/ultrakorne/skillm/releases/latest/download/appcast.xml`), not GitHub Pages. (3) The EdDSA key pair was made with `generate_keys --account skillm` (ran non-interactively; the private key is in this Mac's login keychain, service `https://sparkle-project.org`, account `skillm`); the public key is `SKILLM_UPDATE_PUBLIC_KEY` in `macos/project.yml`, merged into the Info.plist through `macos/Skillm/Info.plist`. `UpdateFeed` refuses a missing, placeholder or non-32-byte key, and a Debug build empties the feed URL, so it never starts Sparkle. (4) "Skip This Version" keeps the item (review fix): Sparkle's silent checks hide a skipped version, but the item's user-started `checkForUpdates` still finds it, so the dot (from the CLI cache) keeps an action. (5) `CURRENT_PROJECT_VERSION` is `$(MARKETING_VERSION)` (review fix): Sparkle compares `CFBundleVersion` with the appcast's `sparkle:version`, which `generate_appcast` copies from the released app's `CFBundleVersion`, so a fixed `1` would never look newer.<br>Review round (`6bca17c`) fixed all 7 confirmed findings: (1, medium) CFBundleVersion fixed at 1 (above; `BundledCLITests` asserts it equals `CFBundleShortVersionString`); (2, medium) Skip This Version left a dot nothing could clear (above; `AppUpgradeTests` updated); (3) an install that failed after the postponed relaunch left the model shut down: `SparkleUpdater` sets `relaunchPending` and, on `didFinishUpdateCycleFor…` with an error, calls the new `AppModel.resumeAfterAbortedUpdate()` (clears the shutdown, restarts the schedule, shows a **user**-origin notice "The update could not be installed", not a background one as the review suggested, since the schedule's immediate first tick would clear a background notice at once); (4) a Sparkle check that did not start (`probe()` now returns `Bool`) or that aborted is no longer recorded for its `checked_at` (`AppUpgrade.probeFailed()`), so the next status (the hourly tick at the latest) asks again; (5) the deviation in (1) is recorded here, plus the optional lookup-error trigger; (6) the Debug "try Sparkle" hint in `project.yml` now names `SKILLM_UPDATE_FEED_URL`, `SKILLM_VERSION=<older release>` and `MARKETING_VERSION`; (7) `SUNoUpdateError` and `SUInstallationCanceledError` aborts log at debug and are not treated as failures. 122 xcodebuild tests pass; a Release build with `MARKETING_VERSION=0.5.0` has `CFBundleVersion` 0.5.0. Docs: new `docs/features/macos-app/updates.md`, the **Appcast** term in `docs/CONTEXT.md`.<br>**Notes for C5:** (a) set **both** `SKILLM_VERSION` and `MARKETING_VERSION` from the tag (`xcodebuild archive … SKILLM_VERSION=0.5.0 MARKETING_VERSION=0.5.0`); `CFBundleVersion` follows `MARKETING_VERSION` automatically, so do not override `CURRENT_PROJECT_VERSION` unless it keeps increasing. (b) Every release must upload `appcast.xml` next to the zipped app (the feed is `releases/latest/download/appcast.xml`); without it Sparkle fails and the app asks again on the next status, at most hourly. Run `generate_appcast --account skillm` (local script: key from the login keychain) or `generate_appcast --ed-key-file -` with the key from a secret (CI); Sparkle's tools (`generate_keys`, `generate_appcast`, `sign_update`) come with the SPM checkout, under `SourcePackages/artifacts/sparkle/Sparkle/bin/` of the derived-data folder. (c) The app asks Sparkle only when the CLI reports `self.available` or a lookup error, and `self.available` needs the GitHub release to carry the darwin archive and `checksums.txt` (goreleaser): release checks must confirm both are present. (d) Sparkle's installer quits the app with an ordinary quit event after `postponeRelaunch`; keep every quit through `applicationShouldTerminate`/`AppDelegate.quit()`. (e) Sign the embedded `Sparkle.framework` (its XPC services, `Autoupdate` and `Updater.app`) with the Developer ID identity before the app (sign inside-out as Sparkle's documentation describes rather than `codesign --deep`, or export the archive with `xcodebuild -exportArchive` and `method: developer-id`).<br>**User actions:** (1) for CI (C5), export the Sparkle private key and store it as a GitHub secret: `<derived data>/SourcePackages/artifacts/sparkle/Sparkle/bin/generate_keys --account skillm -x sparkle_ed_private_key.txt`, then `gh secret set SPARKLE_ED_PRIVATE_KEY < sparkle_ed_private_key.txt` and delete the file. Back the key up: losing it means no signed update can reach installed apps. (2) The Developer ID and notarization actions recorded under A3 still apply to C5 (team 6LH2JMGD3J). |
| C5 | done | def617f26696da61aa7849061ae68c2cd0e3d91f | a44fefd61ce5959e5b99a4e448c450ba3f1d1611 (implementation `d255959`; docs + this log in the next commit) | **Deviations:** (1) One script, `macos/scripts/release.sh [--dry-run] <version>`, is both the local release and the CI job's build step (user decision: local script **and** CI job); `macos/scripts/publish-release.sh <version>` uploads the zip, its `.sha256` and `appcast.xml` to the tag's release (`--clobber`) after checking goreleaser's darwin archives and `checksums.txt` are there. (2) The universal CLI is not a separate `lipo` step: the "Embed skillm CLI" phase (`build-cli.sh`, goreleaser's ldflags) builds both slices during `xcodebuild archive … ONLY_ACTIVE_ARCH=NO MARKETING_VERSION=<v> SKILLM_VERSION=<v>`. (3) The archive is built with `CODE_SIGNING_ALLOWED=NO` and signed by the script inside-out (CLI, Sparkle's `Installer.xpc`, `Downloader.xpc` keeping its entitlements, `Autoupdate`, `Updater.app`, Sparkle, the app; hardened runtime, secure timestamp), not through `-exportArchive`, so CI needs no provisioning or Xcode account; the script refuses `CODE_SIGN_ENTITLEMENTS` in `project.yml`, which this signing would drop. (4) Notarization is `notarytool submit` (ID saved to `notarize/submit.json`) then `notarytool wait --timeout 40m` (review fix), not `submit --wait`; CI authenticates with an App Store Connect API key stored as a notarytool profile in a throwaway keychain. (5) Pre-release tags (with `-`) get no app: the `macos` job runs only for clean `X.Y.Z`. (6) The static `SkillmKit` framework is no longer embedded (`embed: false`), since the copy was unused and would have needed signing; `BundledCLITests` asserts it. (7) Checks beyond the plan: bundle and CLI versions equal the tag, both architectures present, every Mach-O carries the team, hardened runtime and timestamp, the appcast has one item with the zip's URL and length, and its EdDSA signature verifies against the app's `SUPublicEDKey` (`verify-update-signature.swift`). (8) New `appcast-fallback` job (review fix): if the `macos` job fails or is cancelled, it copies the newest earlier release's `appcast.xml` to the new release, so installed apps keep a working feed. goreleaser still makes the release latest before the app job, deliberately not `make_latest: false` as the review suggested: `skillm upgrade` (`internal/selfupdate`) and `install.sh` read `releases/latest`, and holding "latest" back until the app ships would stop CLI releases whenever the app job fails (every tag until the secrets exist).<br>Review round (`a44fefd`) fixed all 4 confirmed findings: (1, high) the identity lookup used an ERE in which `(6LH2JMGD3J)` was a group, so no Developer ID identity ever matched and every release stopped at signing; it is now `macos/scripts/find-identity.sh` (fixed-string `grep -F`), tested against a stub `security` by `macos/scripts/test-find-identity.sh`, which the `macos` job runs first; (2) the missing-appcast gap after a failed job (deviation 8); (3) `release.sh --help` now lists the one-time Mac setup and every CI secret with the commands that create them (the workflow's error points there); (4) the job timeout is 90 minutes, above the 40-minute notarization wait (`SKILLM_NOTARY_TIMEOUT`), and the log upload runs on `failure() || cancelled()`. Gate: Go gate clean, 122 xcodebuild tests pass, `actionlint` and `shellcheck macos/scripts/*.sh` clean, `release.sh --dry-run 0.5.0` passes (signed with Apple Development, no notarization, no appcast without `SPARKLE_ED_PRIVATE_KEY`). Not run for real: this Mac has no Developer ID identity or notary profile, so notarization, stapling, the Gatekeeper check and the keychain-key `generate_appcast` path are untested. Docs: new `docs/features/macos-app/release.md`.<br>**Notes for the next step (the first release):** (a) the first real run is the test of steps 4-8 of `release.sh`: watch `notarize/log.json` and the appcast checks; `spctl` is skipped on CI runners (Gatekeeper off) but runs locally. (b) Run a release locally from a clean checkout of the tag after goreleaser has published it: `git checkout vX.Y.Z && macos/scripts/release.sh vX.Y.Z && macos/scripts/publish-release.sh vX.Y.Z` (macOS may ask once to let `generate_appcast` read the Sparkle key). (c) A red `macos` job does not block the CLI release; rerun it once fixed (`gh run rerun <run-id> --failed`), and its upload replaces the fallback appcast. (d) Add `macos/scripts/test-find-identity.sh`, `shellcheck macos/scripts/*.sh` and `actionlint` to the gate of any step that touches the release scripts. (e) `release.sh` stops if Sparkle's framework layout changes (a Sparkle upgrade): update its signing order then.<br>**User actions:** (1) Create the Developer ID certificate (Account Holder or Admin of team 6LH2JMGD3J): Xcode → Settings → Accounts → Starberry Games GmbH → Manage Certificates → + → Developer ID Application. (2) Local notarization profile: `xcrun notarytool store-credentials skillm-notary --apple-id <apple-id> --team-id 6LH2JMGD3J` (app-specific password from account.apple.com), or `xcrun notarytool store-credentials skillm-notary --key AuthKey_<key id>.p8 --key-id <key id> --issuer <issuer id>`. (3) CI secrets: export the Developer ID identity from Keychain Access (My Certificates → "Developer ID Application: Starberry Games GmbH (6LH2JMGD3J)" → Export, `developer-id.p12`, with a password), then `base64 -i developer-id.p12 | gh secret set MACOS_DEVELOPER_ID_P12_BASE64`, `gh secret set MACOS_DEVELOPER_ID_P12_PASSWORD` (prompts) and delete the file; create an App Store Connect Team API key (Users and Access → Integrations → Team Keys, role Developer), then `gh secret set APPLE_NOTARY_KEY_P8 < AuthKey_<key id>.p8`, `gh secret set APPLE_NOTARY_KEY_ID --body <key id>`, `gh secret set APPLE_NOTARY_ISSUER_ID --body <issuer id>`; and the C4 Sparkle key: `macos/build/SourcePackages/artifacts/sparkle/Sparkle/bin/generate_keys --account skillm -x sparkle_ed_private_key.txt && gh secret set SPARKLE_ED_PRIVATE_KEY < sparkle_ed_private_key.txt && rm sparkle_ed_private_key.txt`. (4) Until those secrets exist, every `v*` tag turns the Release workflow red at "Check the release secrets" (the CLI release still ships). (5) After the first release, install the zip on a colleague's Mac to confirm Gatekeeper accepts it, and at the second release check that the first app offers "Upgrade and restart" and installs it. |
| C6 | in progress | 756aa3278a6d6e05d103110ac3f2081be9ef5676 | | |
