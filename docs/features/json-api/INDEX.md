...
`skillm <command> --json` is skillm's machine-readable surface: one JSON document (or, with `--events`, an NDJSON event stream ending in a result line) on stdout, never a prompt or terminal UI. GUIs such as the macOS menu bar app run the CLI this way instead of parsing its terminal output; `version`, `list` and `check` have a JSON mode.

## Documents
| Document | Purpose |
|----------|---------|
| [DESIGN.md](DESIGN.md) | The contract: flags, envelope, event stream, error codes, and one section per command |
