...
The macOS menu bar app (`macos/`) is a thin SwiftUI view over skillm: it carries its own version-matched **Bundled CLI** and drives it only through the [JSON API](../json-api/INDEX.md), so every behaviour stays in Go. Its status item shows the update Badge as a red dot; its menu shows what the last Refresh found and runs Refresh, the Auto refresh toggle and Update all skills, with Stop and Quit waiting for skillm to exit, and opens the Skills, Add Skill and Settings windows. It asks for a scheduled Refresh at launch, hourly and after a wake, and leaves whether one is Due to the CLI.
