...
// These tests exercise SetAgents' reconcile helpers directly: enabling
// mirrors a peer's link footprint; disabling removes only that agent's links
// and never touches the canonical copies.
...
func TestEnableAgentRecordsUntrackedCwdWithLocalFootprint(t *testing.T) {
	...
	st := &state.State{}
	if _, changed := enableAgent(nil, home, codex, []agentdir.Agent{claude}, st, cwd); !changed {
...
func TestEnableAgentFromHomeMirrorsGlobalOnly(t *testing.T) {
	...
	st := &state.State{}
	enableAgent(nil, home, codex, []agentdir.Agent{claude}, st, globalRoot)
...
func TestEnableAgentSkipsAliasedPeerFootprintAtHome(t *testing.T) {
	...
	st := &state.State{}
	enableAgent(nil, home, weird, []agentdir.Agent{claude}, st, globalRoot)
...
func TestDisableAgentUnlinksButKeepsHome(t *testing.T) {
	...
	st := &state.State{LocalRoots: []string{projA}}
	disableAgent(nil, home, claude, st, cwd)
	...
	if !CopyExists(home, "alpha", agentdir.Global, "") || !CopyExists(home, "beta", agentdir.Global, "") {
