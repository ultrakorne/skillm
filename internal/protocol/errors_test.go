...
func TestErrorFrom(t *testing.T) {
	...
	cases := []struct {
		...
		{"update failed one", &core.UpdateFailedError{IDs: []string{"x"}, Failures: []string{"x: boom"}}, CodeUpdateFailed, "x", "", nil, false},
		{"update failed many", &core.UpdateFailedError{IDs: []string{"x", "y"}, Failures: []string{"x: boom", "y: boom"}}, CodeUpdateFailed, "", "", nil, false},
		{"project dir", fmt.Errorf("import: %w", &core.ProjectDirError{Path: "/p", Err: errors.New("no such file or directory")}), CodeError, "", "/p", nil, false},
